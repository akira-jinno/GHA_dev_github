...
# GitHub providerの設定
provider "github" {
  # GitHub APIにアクセスするためのPersonal Access Token(PAT)を設定。GitHub GUIよりPAT生成してtfvarsで入力。
  token = var.github_token
  # owner: Terraformが操作する対象リソースを指定。ownerを指定しない場合は、tokenに関連付けられたアカウントリソースが対象となる。
  # tokenに入れるPATはGitHubアカウントが対象のため、スコープを組織に絞っておく。組織は手動作成した"cicd-dev-org"を入力。
